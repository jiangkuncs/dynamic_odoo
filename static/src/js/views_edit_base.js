odoo.define('dynamic_odoo.views_edit_base', function (require) {
    ...
    var NewNodeStore = base.WidgetBase.extend({
        ...
        newTitle: function (props = {}) {
            return this.stringXmlToJSON(QWeb.render("Studio.Form.Component.Title", {}));
        },
    ...
    var ViewProps = EditProps.extend({
        init: function (parent, props) {
            ...
            this.nodeProps.widget = {
                name: 'widget', valType: "string", label: '小组件',
                propChange: this.widgetChange.bind(this), widget: basic_fields.ChooseWidget
            };
            this.nodeProps.string = {
                name: 'string', valType: "string", label: '名称', widget: basic_fields.Input,
        ...
        initParams: function () {
            this._super();
            this.tabs = {view: {label: "视图", render: this.renderViewProps.bind(this), name: "view", icon: "th"}};
            this.hideTab = false;
        ...
        prepareParamsProp: function (node, propsView) {
            ...
            if (node) {
                const {viewInfo} = this.props, tagNode = node.tag;
                if (tagNode == "field") {
                    const fieldName = node.attrs.name;
        ...
        prepareDataTabFields: function (search) {
            const {viewInfo} = this.props, fieldsInfo = this.getFieldInfo(), {fields, fieldsGet} = viewInfo,
    ...
    var ViewContent = EditContent.extend({
        ...
        _prepareDataSave: function () {
            const {viewInfo, parentView} = this.props, {view_id, view_key, inParent, model, parentField, type} = viewInfo,
                viewKey = view_key || this.getViewKey(), {action} = $.bbq.getState(true), res = {
                    view_type: this.getViewType(type), action_id: action,
                    view_id: view_id, stack_id: this.stackId, model_name: model, view_key: viewKey
                };
            if (!view_id) {
        ...
        prepareViewParams: function () {
            ...
            const res = {
                ...this.props,
                action: this.action,
                context: {...(context || {}), action: this.action.id || false, STUDIO: true},
                domain: domain || [],
                groupBy: [],
        ...
        renderViewContent: async function () {
            ...
            var viewView = new this.viewConfig.view(viewInfo, this.prepareViewParams()), widget = false;
            if (!viewView.viewSimple) {
                viewView.controllerParams.fromEdit = true;
                viewView.withControlPanel = false;
                viewView.withSearchPanel = false;
                viewView.loadParams.groupedBy = [];
                viewView.loadParams.timeRange = [];
                widget = await viewView.getController(self);
                widget.renderer.isInDOM = true;
                widget.renderer.nodes = self.nodes;
            } else {
                viewView.parent = self;
                widget = viewView;
            }
            self.ref.viewView = viewView;
            self.ref.viewContent = widget;
            return widget.appendTo(self.$el.find(".viewView").empty()).then(() => {
