odoo.define('dynamic_odoo.AppCenter', function (require) {
    ...
    var IconCreator = base.WidgetBase.extend({
        ...
        iconHtmlToData: async function () {
            const dataUrl = await (domtoimage || {toPng: async () => {}}).toPng(document.getElementsByClassName("IC")[0]);
            return dataUrl;
        },
    ...
    var BaseChooseView = base.WidgetBase.extend({
        ...
        onCreate: async function (e) {
            e.stopPropagation();
            const self = this, {first, second, iconCreator} = this.ref, data = first.getData();
            ...
            return this['_rpc']({
                ...
                if (data) {
                    self.trigger_up('onShowNewApp', data);
                }
            })
        },
        bindAction: function () {
