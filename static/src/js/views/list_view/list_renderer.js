odoo.define('dynamic_odoo.ListRenderer', function (require) {
    ...
    patch(components.SearchBar, 'dynamic_odoo.ListRenderer', {
        _onFacetRemove(facet) {
            ...
            this.trigger("facet_remove", {filters: filters});
        }
    });

    ListController.include({
        custom_events: _.extend({}, ListController.prototype.custom_events, {
            'facet_remove': 'facetRemove',
            'on_search': 'onSearchAdvance'
        }),
        init: function (parent, model, renderer, params) {
            this._super(parent, model, renderer, params);
        },
        onSearchAdvance: function (ev) {
            const {group, filters} = ev.data;
            if (group) {
        ...
        facetRemove: function (ev) {
            const {filters} = ev.data || {};
            if (filters && filters.length) {
