odoo.define('dynamic_odoo.form', function (require) {
    ...
    var FormEditRenderer = FormRenderer.extend({
        ...
        onNewPage: function (nodeNotebook) {
            ...
            this.ref.nodes[nodeNotebook.nodeId].state.value = newPage.attrs.name;
        ...
        _renderWrapEditO2m: function (fieldInfo) {
            return QWeb.render("Edit.wrapAb", {
                name: fieldInfo.name, views: [{type: "list", label: "编辑列表"}, {type: "form", label: "编辑表单"}]
            });
        },
    ...
    var FormEditView = FormView.extend({
        ...
        init: function (viewInfo, params) {
            ...
            const {newNodes, setNodeId, bindAction, setNodeActive} = params;
    ...
    var FormProps = baseEdit.ViewProps.extend({
        init: function (parent, props) {
            ...
            this.nodeProps.help = {name: 'help', valType: "string", label: '帮助', widget: basic_fields.Input};
            this.nodeProps.context = {name: 'context', valType: "string", label: '上下文', widget: basic_fields.Input};
            ...
            this.nodeProps.relation = {
                name: 'relation', valType: "string", label: '关系', widget: basic_fields.Relation,
            ...
            this.nodeProps.col = {name: 'col', valType: "string", label: '列祖', widget: basic_fields.Input};
            this.nodeProps.columns = {name: 'columns', valType: "string", label: '列', widget: basic_fields.Input};
            this.nodeProps.colspan = {name: 'colspan', valType: "string", label: '行', widget: basic_fields.Input};
            ...
            this.nodeProps.invisible_view_mode = {
                name: 'invisible_view_mode', valType: "boolean", label: '在视图模式下不可见',
            ...
            this.nodeProps.approval_groups = Object.assign({}, this.nodeProps.groups, {
                ...
                label: "审批组", no_xml: true, valType: "list", propChange: this.changePropDetails.bind(this)
            });
        ...
        initParams: function () {
            ...
            this.components = {
                component: {label: "标签", class: "_wComTag", type: "component"},
                fields: {label: "字段", class: "_wComField", type: "fieldNew"}
            };
            this.components.component.child = {
                title: {name: "title", label: "标题", icon: "gg"},
                text: {name: "text", label: "文本", icon: "font"},
                image: {name: "image", label: "图片", icon: "image"},
                button: {name: "button", label: "按钮", icon: "gg"},
                grid: {name: "grid", label: "网格", icon: "th-large"},
                group: {name: "group", label: "分组", icon: "object-group"},
                notebook: {name: "notebook", label: "笔记本", icon: "leanpub"}
            };
            this.components.fields.child = {
                datetime: {name: "datetime", label: "日期时间", icon: "clock-o"},
                date: {name: "date", label: "日期", icon: "calendar"},
                char: {name: "char", label: "字符", icon: "font"},
                text: {name: "text", label: "文本", icon: "arrows-alt"},
                many2one: {name: "many2one", label: "多对一", icon: "envira"},
                one2many: {name: "one2many", label: "一对多", icon: "bars"},
                many2many: {name: "many2many", label: "多对多", icon: "pagelines"},
                integer: {name: "integer", label: "数字", icon: "yelp"},
                monetary: {
                    name: "monetary",
                    label: "货币",
                    icon: "modx",
                ...
                float: {name: "float", label: "浮点", icon: "fire"},
                binary: {name: "binary", label: "二进制", icon: "file"},
                selection: {name: "selection", label: "选择", icon: "file"},
                boolean: {name: "boolean", label: "布尔", icon: "check-square"},
            };
        },
        ...
        prepareFieldView: function (node) {
            const {fields, viewFields} = this.props.viewInfo, {name} = node.attrs,
    ...
    var FormEditContent = baseEdit.ViewContent.extend({
        ...
        _changeStateData: function (fieldName, fieldInfo) {
            const {model, handle, renderer, viewType} = this.ref.viewContent, state = model.localData[handle],
        ...
        prepareNewNode: function (props) {
            ...
            if (node.attrs.component == "title") {
                this.ref.viewView._processArch(node, viewInfo);
            }
            if (props.props._new) {
                ...
                this.ref.viewView._processNode(node, viewInfo);
        ...
        onShowSubView: async function (e) {
            ...
            if (renderSubView) {
                ...
                if (!subViewInfo) {
                    ...
                    subViewInfo = await this.ref.viewContent.loadFieldView(relation, _context, false, viewType, {});
                    subViewInfo = this.ref.viewView._processFieldsView(subViewInfo, viewType);
                ...
                renderSubView(fieldInfo.views[viewType]);
            }
        },
        ...
        onChangeProp: function (node) {
            const self = this;
            // if (node.attrs._reload || node._reload) {
            //     delete node._reload, delete node.attrs._reload;
            //     return this._super(node).then(() => {
            //         self.onClickNode(self.nodes[node.nodeId]);
            //     });
            // }
        ...
        reloadNode: function (nodeReload) {
            ...
            return this.ref.viewContent.renderer.reloadNode(nodeReload).then(() => {
                self.bindAction();
                self.bindStyle();
            });
        },
