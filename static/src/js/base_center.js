odoo.define('dynamic_odoo.BaseCenter', function (require) {
    ...
    var ControlModel = BasicModel.extend({
        ...
        getView: function (domain) {
            let _domain = [[]]
            if (domain.length == 1 && domain[0] != undefined) {
                _domain = domain
            }
            return this['_rpc']({
                model: "studio.view.center",
    ...
    var BaseChooseView = base.WidgetBase.extend({
        ...
        onClose: async function (e) {
            // FIX START
            // 从 do_action 打开的页面，没有 action_id
            // NEW
            if (e) {
                e.stopPropagation();
            ...
            const options = {clear_breadcrumbs: true, viewType: viewType};
            if (!state.action) {
                ...
                if (viewType == "form") {
                    ...
                    options.index = 0;
                    action.res_id = state.id;
                }
                core.bus.trigger('clear_cache');
                await this.do_action(action, options);
            }
            ...
            //     e.stopImmediatePropagation();
            // }
            // const action = await this.getAction(), {viewType} = this.state, state = $.bbq.getState(true),
            //     options = {clear_breadcrumbs: true, viewType: viewType};
            // if (viewType == "form") {
            //     options.clear_breadcrumbs = false;
            //     options.index = 0;
            //     action.res_id = state.id;
        ...
        getAction: function () {
            const self = this, state = $.bbq.getState(true), {model, action} = state,
                options = {active_model: model, STUDIO: Math.random()};
            this.action_manager = new ActionManager(this, session.user_context);
            this.action_manager.isInDOM = true;
            if (state.id) {
                options.active_id = state.id;
                options.active_ids = [state.id];
            }
            return self.action_manager._loadAction(action, options).then(function (action) {
        ...
        renderNewApp: function () {
            const appCenter = new AppCenter(this, {});
            appCenter.appendTo(this.$el.find(".wgCon").empty());
        },
